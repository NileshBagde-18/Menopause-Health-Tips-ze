"use client"

import { useState } from "react"
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
import { Button } from "@/components/ui/button"
import { Progress } from "@/components/ui/progress"
import { Calendar, BookOpen, Zap, Award, Flame, Star, CheckCircle, Heart, Moon, Thermometer } from "lucide-react"

export function ToolsTab() {
  const [selectedAffirmation, setSelectedAffirmation] = useState(0)
  const [weeklyCheckIn, setWeeklyCheckIn] = useState({
    hotFlashes: 3,
    sleepQuality: 7,
    moodRating: 6,
    energyLevel: 5,
  })

  const affirmations = [
    "I am strong and capable of handling any challenge that comes my way.",
    "My body is wise and knows how to heal and balance itself.",
    "I embrace this new chapter of my life with grace and confidence.",
    "I deserve love, care, and compassion - especially from myself.",
  ]

  const journalEntries = [
    { date: "Today", mood: "😊", entry: "Feeling more energetic after trying the new morning routine..." },
    { date: "Yesterday", mood: "😐", entry: "Had a few hot flashes but managed them well with breathing..." },
    { date: "2 days ago", mood: "😌", entry: "Great sleep last night! The magnesium supplement seems to help..." },
  ]

  return (
    <div className="flex flex-col h-full bg-gray-50">
      {/* Header */}
      <div className="flex-shrink-0 text-center p-4 bg-white border-b border-gray-200">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">Your Wellness Tools</h1>
        <p className="text-gray-600">Track, reflect, and grow on your journey</p>
      </div>

      {/* Scrollable Content */}
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        {/* Gamification Stats */}
        <Card className="bg-gradient-to-r from-purple-500 to-purple-600 text-white border-0">
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-9">
              <div>
                <h3 className="text-lg font-semibold">Your Progress</h3>
                <p className="text-purple-100 tracking-normal">Keep up the great work!</p>
              </div>
              <div className="text-right">
                <div className="font-bold text-lg">1,247</div>
                <div className="text-sm text-purple-100">Total Points</div>
              </div>
            </div>

            <div className="grid grid-cols-3 gap-4">
              <div className="text-center">
                <Flame className="w-6 h-6 mx-auto mb-1 text-orange-300" />
                <div className="text-lg font-bold">12</div>
                <div className="text-xs text-purple-100">Day Streak</div>
              </div>
              <div className="text-center">
                <Award className="w-6 h-6 mx-auto mb-1 text-yellow-300" />
                <div className="text-lg font-bold">8</div>
                <div className="text-xs text-purple-100">Badges</div>
              </div>
              <div className="text-center">
                <Star className="w-6 h-6 mx-auto mb-1 text-yellow-300" />
                <div className="text-lg font-bold">Gold</div>
                <div className="text-xs text-purple-100">Level</div>
              </div>
            </div>
          </CardContent>
        </Card>

        {/* Daily Affirmations */}
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Zap className="w-5 h-5 text-purple-500" />
              Daily Affirmations
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="rounded-lg p-4 mb-4 bg-purple-100">
              <p className="text-gray-800 italic text-center leading-relaxed text-xl">"{affirmations[selectedAffirmation]}"</p>
            </div>

            <div className="flex justify-between items-center">
              <Button
                variant="outline"
                size="sm"
                onClick={() => setSelectedAffirmation((prev) => (prev - 1 + affirmations.length) % affirmations.length)}
              >
                Previous
              </Button>
              <div className="flex gap-2">
                {affirmations.map((_, index) => (
                  <div
                    key={index}
                    className={`w-2 h-2 rounded-full ${index === selectedAffirmation ? "bg-purple-500" : "bg-gray-300"}`}
                  />
                ))}
              </div>
              <Button
                variant="outline"
                size="sm"
                onClick={() => setSelectedAffirmation((prev) => (prev + 1) % affirmations.length)}
              >
                Next
              </Button>
            </div>

            <div className="mt-4 flex gap-2">
              <Button className="flex-1 bg-purple-500 hover:bg-purple-600">
                <Heart className="w-4 h-4 mr-2" />
                Save to Favorites
              </Button>
              <Button variant="outline" className="flex-1 bg-transparent">
                Share
              </Button>
            </div>
          </CardContent>
        </Card>

        {/* Weekly Check-In */}
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Calendar className="w-5 h-5 text-blue-500" />
              Weekly Check-In
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              <div className="flex justify-between items-center mb-2">
                <span className="text-sm font-medium flex items-center gap-2">
                  <Thermometer className="w-4 h-4 text-red-500" />
                  Hot Flashes (per day)
                </span>
                <span className="text-sm text-gray-600">{weeklyCheckIn.hotFlashes}</span>
              </div>
              <Progress value={weeklyCheckIn.hotFlashes * 10} className="h-2" />
            </div>

            <div>
              <div className="flex justify-between items-center mb-2">
                <span className="text-sm font-medium flex items-center gap-2">
                  <Moon className="w-4 h-4 text-indigo-500" />
                  Sleep Quality
                </span>
                <span className="text-sm text-gray-600">{weeklyCheckIn.sleepQuality}/10</span>
              </div>
              <Progress value={weeklyCheckIn.sleepQuality * 10} className="h-2" />
            </div>

            <div>
              <div className="flex justify-between items-center mb-2">
                <span className="text-sm font-medium flex items-center gap-2">
                  <Heart className="w-4 h-4 text-pink-500" />
                  Mood Rating
                </span>
                <span className="text-sm text-gray-600">{weeklyCheckIn.moodRating}/10</span>
              </div>
              <Progress value={weeklyCheckIn.moodRating * 10} className="h-2" />
            </div>

            <Button className="w-full mt-4">
              <CheckCircle className="w-4 h-4 mr-2" />
              Complete This Week's Check-In
            </Button>
          </CardContent>
        </Card>

        {/* Personal Journal */}
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <BookOpen className="w-5 h-5 text-green-500" />
              Personal Journal
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-3 mb-4">
              {journalEntries.map((entry, index) => (
                <div key={index} className="bg-gray-50 rounded-lg p-3">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-medium text-gray-700">{entry.date}</span>
                    <span className="text-lg">{entry.mood}</span>
                  </div>
                  <p className="text-sm text-gray-600">{entry.entry}</p>
                </div>
              ))}
            </div>

            <div className="flex gap-2">
              <Button className="flex-1">
                <BookOpen className="w-4 h-4 mr-2" />
                New Entry
              </Button>
              <Button variant="outline">Export</Button>
            </div>
          </CardContent>
        </Card>
      </div>
    </div>
  )
}
